<template>
  <div class="flex flex-col md:flex-row items-center justify-center w-full h-full">
    <div class="w-full md:w-1/2 flex flex-col items-center justify-center p-4">
      <div v-if="isLoadingInforme" class="text-center">Cargando informe...</div>
    ...
    <!-- Progress modal (error no bloquea vista) -->
    <div
      ...
      <div class="flex flex-col gap-4">
        <div class="flex items-start justify-between flex-col sm:flex-row gap-2 sm:gap-0">
          <div>
            ...
            <div class="text-xs text-gray-500">
              Sumatoria de (<em>cantidad-de-entradas * costo-de-entrada</em>) de todo el inventario
              en CUP.
            </div>
          </div>
        ...
        <div class="flex items-start justify-between flex-col sm:flex-row gap-2 sm:gap-0">
          <div>
            <div class="flex items-center gap-3">
              ...
              <span class="font-medium">Inventario (valor CUP)</span>
            </div>
            <div class="text-xs text-gray-500">
              Sumatoria de (<em>costo * cantidad-en-existencia</em>) de todos los productos en el
              sistema (en CUP).
            </div>
          </div>
          <div class="font-semibold text-right">
        ...
        <div class="flex items-start justify-between flex-col sm:flex-row gap-2 sm:gap-0">
          <div>
            <div class="flex items-center gap-3">
              ...
              <span class="font-medium">Ventas (CUP)</span>
            </div>
            <div class="text-xs text-gray-500">
              Sumatoria de (<em>precio-cobrado * cantidad</em>) de todas las ventas (en CUP, sin
              conversiones).
            </div>
          </div>
          <div class="font-semibold text-right">
        ...
        <div class="flex items-start justify-between flex-col sm:flex-row gap-2 sm:gap-0">
          <div>
            ...
            <div class="text-xs text-gray-500">
              Sumatoria de (<em>cantidad * costo_producto</em>) de las salidas (pérdidas) en CUP.
            </div>
          </div>
          <div class="font-semibold text-right">
        ...
        <div class="flex items-start justify-between flex-col sm:flex-row gap-2 sm:gap-0">
          <div>
            <div class="flex items-center gap-3">
              ...
              <span class="font-medium">Retiros (CUP)</span>
            </div>
            <div class="text-xs text-gray-500">
              Sumatoria en CUP tomada directamente de los retiros en CUP (sin usar
              <code>cambio_moneda</code>).
            </div>
          </div>
        ...
        <div class="flex items-start justify-between flex-col sm:flex-row gap-2 sm:gap-0">
          <div>
            <div class="flex items-center gap-3">
              ...
              <span class="font-medium">Costo Ventas (CUP)</span>
            </div>
            <div class="text-xs text-gray-500">
              Sumatoria de (<em>costo-venta * cantidad</em>) de todas las ventas registradas en CUP.
            </div>
          </div>
        ...
        <div class="flex items-start justify-between flex-col sm:flex-row gap-2 sm:gap-0">
          <div>
            <div class="flex items-center gap-3">
              <span class="w-4 h-4 rounded-sm border border-gray-300 inline-block"></span>
              <span class="font-medium">Ganancia (Ventas - Costos - Retiros) CUP</span>
            </div>
            <div class="text-xs text-gray-500">
              Ganancia neta en CUP:
...
const isLoadingInforme = ref(false);
...
async function generateInformeCup() {
  isLoadingInforme.value = true;
  ...
  try {
    ...
    try {
      ...
      for (const e of entradas || []) {
        // Para CUP asumimos que el backend ya devuelve el costo en CUP;
        // si existe campo *_cup se usa, en su defecto se reutiliza el actual.
        const costoEntrada =
          safeNum(e.costo);
        const cantidad = safeNum(e.cantidadEntrada);
        compras += cantidad * costoEntrada;
      }
    ...
    try {
      ...
      for (const p of productos || []) {
        ...
        inventario += costoProducto * cantidadExist;
      }
    ...
    try {
      ...
      for (const v of ventas || []) {
        // CUP: usamos directamente el precio cobrado sin dividir por tipos de cambio.
        const precioCobradoCup =
          safeNum(v.precio_cobrado_cup ?? v.precio_cobrado) || 0;
        const costoVentaCup =
          safeNum(v.costo_venta) || 0;
        const cantidad = safeNum(v.cantidad);

        ventasSum += precioCobradoCup * cantidad;
        costoVentasSum += costoVentaCup * cantidad;
      }
    ...
    try {
      ...
      for (const s of salidas || []) {
        ...
        const costoSalidaCup =
          safeNum(s.costo_producto_cup);
        salidasSum += cantidad * costoSalidaCup;
      }
    ...
    // 5) /Retiro/filterRetiros - CUP puro: solo tomamos cantidad_retirada_cup sin usar cambio_moneda
    let retiros = [];
    try {
    ...
    try {
      ...
      for (const r of retiros || []) {
        // Solo se usa el valor en CUP directamente; se ignora cambio_moneda y cantidad_retirada_usd
    ...
    console.error('Error generating informe CUP:', err);
    metricsError.value = true;
    metricsErrorMessage.value = 'Ocurrió un error generando el informe CUP';
  } finally {
    isLoadingInforme.value = false;
...
const pieStyle = computed(() => {
  // Mostrar en el gráfico las categorías: compras, inventario, ventas, salidas
  const visibleValues = [
    comprasTotal.value,
    inventarioTotal.value,
    ventasTotal.value,
...
// Ganancia: ventas - costo de ventas - pérdidas - retiros
...
onMounted(() => {
  // Ejecutar informe CUP solo cuando el componente se monta
  generateInformeCup();
});
</script>
